...
export const authService = {
  async emailAvailable(
    refetch: any,
    email: string,
  ): Promise<{ available: boolean; message: string; success: boolean }> {
    ...
    try {
      ...
      return {
        available: false,
        message: response.data.message || "Email is already registered",
        success: true,
      };
    } catch (error) {
  ...
  async onboardingStatus(
    refetch: any,
    email: string,
  ): Promise<{ needsOnboarding: boolean; message: string; success: boolean }> {
    ...
    try {
      ...
      return {
        needsOnboarding: false,
        message: `Error checking onboarding status: ${error}`,
        success: false,
      };
    }
  },
  ...
  async getProvider(
    refetch: any,
    email: string,
  ): Promise<{ provider: string | null; message: string; success: boolean }> {
    ...
    try {
      ...
      const provider =
        response.data.providers ?? response.data.provider ?? null;
      ...
      return {
        provider: null,
        message: `Error retrieving provider: ${error}`,
        success: false,
      };
    }
  },
  ...
  async changePassword(
    ...
    logger.log(`Changing password for: ${email}`);
    try {
      const response = await refetch({ body: { email, newPassword } });
      logger.debug("Response received", response);
