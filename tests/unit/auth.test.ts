...
describe("authService", () => {
  ...
  describe("emailAvailable", () => {
    it("returns success when email is available", async () => {
      ...
      const result = await authService.emailAvailable(
        mockRefetch,
        "test@example.com",
      );
    ...
    it("handles multiple sequential calls", async () => {
      ...
      const r1 = await authService.emailAvailable(
        mockRefetch,
        "test1@example.com",
      );
      const r2 = await authService.emailAvailable(
        mockRefetch,
        "test2@example.com",
      );
  ...
  describe("onboardingStatus", () => {
    it("returns true when user needs onboarding", async () => {
      ...
      const result = await authService.onboardingStatus(
        mockRefetch,
        "test@example.com",
      );
      expect(result).toEqual({
    ...
    it("handles multiple sequential calls", async () => {
      ...
      const r1 = await authService.onboardingStatus(
        mockRefetch,
        "new@example.com",
      );
      const r2 = await authService.onboardingStatus(
        mockRefetch,
        "existing@example.com",
      );
  ...
  describe("resetPassword", () => {
    ...
    it("handles non-200 status", async () => {
      ...
      const result = await authService.resetPassword(mockRefetch, "newPass");
      expect(result.success).toBe(false);
    });
    ...
    it("handles multiple sequential calls", async () => {
      ...
      const r1 = await authService.resetPassword(mockRefetch, "p1");
      const r2 = await authService.resetPassword(mockRefetch, "p2");
      expect(r1.success).toBe(true);
  ...
  describe("createAccount", () => {
    ...
    it("handles multiple sequential calls", async () => {
      ...
      const r1 = await authService.createAccount(mockRefetch, {
        email: "u1@ex.com",
      });
      const r2 = await authService.createAccount(mockRefetch, {
        email: "u2@ex.com",
      });
      expect(r1.success).toBe(true);
      expect(r2.success).toBe(false);
  ...
  describe("verifyOtp", () => {
    ...
    it("handles multiple sequential calls", async () => {
      ...
      const r1 = await authService.verifyOtp(
        mockRefetch,
        "u1@ex.com",
        "123",
        "signup",
      );
      const r2 = await authService.verifyOtp(
        mockRefetch,
        "u2@ex.com",
        "000",
        "signup",
  ...
  describe("Checkbox Validation Logic", () => {
    ...
    it("returns false when both unchecked", () =>
      expect(validateCheckboxes(false, false)).toBe(false));
    it("returns false when only Terms is checked", () =>
      expect(validateCheckboxes(true, false)).toBe(false));
    it("returns false when only Privacy Policy is checked", () =>
      expect(validateCheckboxes(false, true)).toBe(false));
    it("returns true when both are checked", () =>
      expect(validateCheckboxes(true, true)).toBe(true));
  });
  ...
  describe("Next Button Enable Logic", () => {
    ...
    it("is disabled when email is empty", () =>
      expect(isNextButtonEnabled("", true)).toBe(false));
    it("is disabled when email is invalid", () =>
      expect(isNextButtonEnabled("a@a.com", false)).toBe(false));
    it("is disabled when both conditions fail", () =>
      expect(isNextButtonEnabled("", false)).toBe(false));
    it("is enabled when email is valid and not empty", () =>
      expect(isNextButtonEnabled("a@a.com", true)).toBe(true));
  });
  ...
  describe("Google Button Enable Logic", () => {
    ...
    it("is disabled when both unchecked", () =>
      expect(isGoogleButtonEnabled(false, false)).toBe(false));
    ...
    it("is enabled when both are checked", () =>
      expect(isGoogleButtonEnabled(true, true)).toBe(true));
  });

  describe("Email Change Detection Logic", () => {
    const hasEmailChanged = (s: string | null, c: string) =>
      s !== null && s !== c;
    it("returns false when no stored email", () =>
      expect(hasEmailChanged(null, "a@a.com")).toBe(false));
    it("returns false when emails match", () =>
      expect(hasEmailChanged("a@a.com", "a@a.com")).toBe(false));
    it("returns true when emails differ", () =>
      expect(hasEmailChanged("o@o.com", "n@n.com")).toBe(true));
    it("is case-sensitive", () =>
      expect(hasEmailChanged("A@a.com", "a@a.com")).toBe(true));
  });
  ...
  describe("Onboarding Decision Logic", () => {
    ...
    it("routes to createUserInfo for Google users needing onboarding", () =>
      expect(determineNextRoute(true, true)).toBe("createUserInfo"));
    it("routes to emailOTP for regular users needing onboarding", () =>
      expect(determineNextRoute(true, false)).toBe("emailOTP"));
  ...
  describe("Error Message Formatting", () => {
    const formatErrorMessage = (error: any): string => {
      if (error instanceof Error)
        return `Error checking email existence: ${error.message}`;
      return `Error checking email existence: ${error}`;
    };
    it("formats Error objects", () => {
      expect(formatErrorMessage(new Error("fail"))).toBe(
        "Error checking email existence: fail",
      );
    });
    it("formats string errors", () =>
      expect(formatErrorMessage("oops")).toContain("oops"));
    it("handles unknown error types", () =>
      expect(formatErrorMessage({ code: 500 })).toContain(
